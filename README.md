...
在我的设备Windows+安卓上实际测试，release内的功能和bug修复均完成。
...
- 修复虎牙 / B站直播间在没有头条或 SC 时无任何提示的问题，补上明确空状态。
- 修复直播间内“正在连接弹幕服务器”“线路选择”“线路 N”等中文文案乱码，并继续统一按 `UTF-8` 维护中文文案与注释。
- 修复直播间内通过推荐 / 历史再次进房时出现的页面套娃、返回黑屏和房间切换异常问题。
- 修复直播间内关注列表只显示开播主播的问题，补齐“全部 / 直播中 / 未开播”筛选能力。
- 修复 B站贡献榜重复更新、刷新异常的问题，并改进贡献榜按需刷新逻辑。
...
- 直播间内点击用户名或整条弹幕都可进行屏蔽、临时禁言、备注、复制、批量恢复等操作，并支持复制弹幕内容。
- 新增 `B站 / 抖音 / 斗鱼` 贡献榜或亲密榜展示能力。
- 同类推荐、观看历史、用户快捷操作等直播间交互继续补强，并避免在直播间内部重复套娃导航。

### Windows

- 修复直播间标题居中逻辑被右侧操作栏影响的问题，改为按实际直播画面区域计算视觉中心。
- 修复 Windows 小窗、全屏、窗口恢复、鼠标自动隐藏、悬浮控制区闪动等一系列桌面播放器问题。

### Android

- 修复安卓后台恢复与直播间恢复逻辑，提升应用被系统回收后的可恢复性。
- 修复安卓端全屏下从历史 / 关注 / 推荐打开右侧列表时一闪就消失的问题。
- 修复“我的”里的直播设置、弹幕设置等长页面与安卓虚拟导航栏重叠的问题。

### Android TV

- TV 更新检查已切换到当前公开仓库的版本说明与下载页，不再继续指向上游仓库。
- TV 设置页补充黑屏排查提示，方便在没有设备的情况下给用户明确的兼容性操作建议。
- TV 播放已在本地 `Android Emulator - Medium_Phone`（`Android 16 / API 36 / x86_64`）验证通过，虎牙、斗鱼、抖音均正常出画面；当前建议保持 `硬件解码` 开启。
...
- Windows / Android / TV 本地构建链统一收敛回单路径 Flutter，减少多套 SDK 混跑导致的路径污染问题。

## 下一步计划

### 全平台通用

- 继续补完部分平台接口细节，例如分区字段、标签图标、推荐逻辑反查和直播间周边数据一致性。
- 把弹幕显示行数改为根据显示区域自适应弹幕上下间距，并设置最低间距防止过于重叠，以及最大间距防止显示不好看和超出显示区域，即让显示的行数贴合显示的区域。
- 继续研究虎牙贡献榜的稳定方案，如果网页端不稳定，再评估 APP 侧接口或保底方案。
- 继续优化聊天区自动滚动、推荐直播、观看历史和贡献榜筛选等直播间交互体验。

### Android

- 继续补强安卓直播间底部按钮与虚拟导航栏重叠、后台长时间挂起后的恢复细节。

### Android TV

- 继续排查 Android TV 设备兼容性，重点关注黑屏、有声无画和硬件解码差异问题。
